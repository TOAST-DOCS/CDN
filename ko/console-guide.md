...
#### 설명 { #basic-information-description }
CDN 서비스의 설명을 추가합니다.

<a id="basic-information-domain-alias"></a>
#### 도메인 별칭 { #basic-information-domain-alias }
...
#### 원본 경로 { #origin-server-original-path }
...
#### 원본 요청 HTTP 프로토콜 다운그레이드 { #origin-server-downgrading-http-protocols-requesting-originals }
...
#### 루트 경로 접근 설정 { #controlling-the-access-of-root-path-set-root-path-access }
...
#### 접근 제어 방식 { #controlling-the-access-of-root-path-access-control-method }
...
#### Redirect 경로 { #controlling-the-access-of-root-path-redirect-path }
...
#### 메서드 허용 설정 { #method-allow-method-settings }
...
#### 캐시 설정 { #cache-configuration-of-cache }
...
#### 접근 제어 방식 { #access-management-for-referer-header-access-control-types }
- 블랙리스트(blacklist) 타입
...
#### 리퍼러 헤더가 없는 경우 접근 허용 { #access-management-for-referer-header-content-access-if-referer-header-is-unavailable }
...
#### 1. NHN Cloud CDN 콘솔 > Auth Token 인증 접근 관리 설정 { #access-control-for-auth-token-authentication-nhn-cloud-cdn-console-access-control-settings-for-auth-token-authentication }
...
#### 2. 토큰 생성 { #access-control-for-auth-token-authentication-create-a-token }
...
#### 3. 생성된 토큰을 콘텐츠 요청에 포함 { #access-control-for-auth-token-authentication-include-created-token-to-the-request-of-content }
...
#### HTTP 응답 헤더 설정 { #http-response-header-settings }
...
#### DNS TXT 레코드 추가 방식 { #verify-a-domain-dns-txt-record-addition-method }
...
#### HTTP 파일 인증 방식 { #verify-a-domain-http-file-authentication-method }
...
#### HTTP 리다이렉트 인증 방식 { #verify-a-domain-http-redirect-authentication-method }
...
#### DNS TXT 레코드 추가 방식 { #validate-domain-adding-dns-txt-records }
...
#### HTTP 페이지 추가 방식 { #validate-domain-adding-http-pages }
