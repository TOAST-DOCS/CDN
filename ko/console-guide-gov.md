...
> **[주의] 도메인 등록 시 확인 사항**
> 1. 전체 도메인 주소(FQDN) 형식만 입력할 수 있습니다(예: cdn.example.com).
> 2. 이미 등록된 도메인은 중복으로 등록할 수 없습니다.

### 도메인 검증
등록한 도메인의 소유권을 확인하기 위해 도메인 검증을 진행해야 합니다.  
도메인 검증 방식에는 **DNS TXT 레코드 추가**, **HTTP 파일 인증**, **HTTP 리다이렉트 인증** 세 가지 방식이 있으며, 세 가지 방식 중 **하나만 진행**하면 됩니다.  
...
> **[주의]** HTTP 파일 인증 방식은 HTTP 기본 포트(80)로 운영 중인 웹 서버에서만 사용할 수 있습니다.
...
> **[주의] 도메인 검증 주의 사항**
> 1. 도메인 검증 토큰은 등록일로부터 **14일** 동안 유효합니다. 기간 내 검증을 완료하지 않으면 토큰이 만료되며, **토큰 재발급** 후 다시 검증해야 합니다.
> 2. 토큰이 만료된 경우 콘솔에서 **토큰 재발급** 버튼을 클릭하여 새 검증 토큰을 발급받을 수 있습니다. 토큰이 재발급되면 이전 검증 정보는 초기화되므로 새 토큰 정보로 다시 검증을 진행해야 합니다.
> 3. 도메인 검증이 완료되면 상태가 **검증 완료**로 변경되며, CDN 서비스에 연동할 수 있습니다.
...
| 토큰 만료   | 검증 토큰이 만료된 상태(토큰 재발급 후 다시 검증 필요)                      |
...
2. **기본 정보**의 **도메인 별칭** 항목에서 검증이 완료된 도메인을 선택합니다.
    - 검증이 완료된 도메인만 선택할 수 있습니다. 설정하고자하는 도메인이 목록에 표시되지 않으면 **도메인 별칭** 탭에서 도메인을 먼저 등록하고 검증을 완료해 주세요.
3. **확인** 버튼을 클릭하여 변경을 완료합니다.
4. 도메인 별칭 설정 후에는 도메인의 DNS 서비스 제공 업체에서 CNAME 레코드를 다음과 같이 설정해야 합니다. DNS 설정 관련 문의는 DNS 서비스 제공 업체에 하시기 바랍니다.
    - 레코드 타입: **CNAME**
    - 레코드 이름: **[도메인 별칭에 등록한 도메인]**
    - 레코드값(Rdata): **[서비스ID].toastcdn.net**
    - TTL: 임의의 값
...
> 인증서를 계속 유지하려면 도메인 별칭을 다시 추가하여 인증한 후, CDN 서비스에 다시 도메인 별칭으로 연동하시기 바랍니다.
...
> **[주의] 도메인 삭제 시 확인 사항**
> CDN 서비스에 연동된 도메인은 삭제할 수 없습니다. 도메인을 삭제하려면 먼저 CDN 서비스에서 도메인 별칭 연동을 해제한 후 삭제하시기 바랍니다.
